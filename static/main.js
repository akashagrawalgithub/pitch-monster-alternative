...
function initializeSpeechify() {
    if (typeof window.Speechify !== 'undefined') {
        window.Speechify.init({
...
async function getAudioBufferFromSpeechify(text) {
    return new Promise((resolve, reject) => {
        if (typeof window.Speechify === 'undefined') {
            reject(new Error('Speechify SDK not loaded'));
            return;
        }
        window.Speechify.speak({
            text: text,
            voice: 'steven',
            format: 'mp3',
            onStart: () => {
...
async function speakWithSDK(text) {
    return new Promise((resolve, reject) => {
        if (typeof window.Speechify === 'undefined') {
            reject(new Error('Speechify SDK not loaded'));
            return;
        }
        window.Speechify.stream({
            text: text,
            voice: 'steven',
            onChunk: (chunk) => {
...
async function getAudioBufferFromSpeechifyAPI(text) {
    return new Promise((resolve, reject) => {
        fetch('https://api.sws.speechify.com/v1/audio/stream', {
            method: 'POST',
            headers: {
...
function startContinuousAudioPlayback() {
    if (isAudioPlaying || audioBufferQueue.length === 0 || !audioContext)
        return;
    isAudioPlaying = true;
    playNextAudioBuffer();
}
