            ...
            speechPauseTimeout = window.setTimeout(() => {
                if (finalTranscript.trim().length > 0) {
                    ...
                    if (speechToProcess.length > 0) {
                        ...
                        const interimBubbleElement = document.querySelector('.transcript-item.you .transcript-content');
...
async function playBase64Audio(base64, format = 'pcm_f32le', sampleRate = 44100) {
...
async function playAudioBuffer(buffer, sampleRate) {
    if (!audioContext) {
...
function shouldCreateChunk(text) {
    ...
    if (text.length > 150)
        return true; // Reduced from 200 to 150 for faster processing
    return false;
}
...
async function playNextInQueue() {
    if (audioPlaybackQueue.length === 0 || isPlayingQueue)
        return;
    isPlayingQueue = true;
...
function addStreamingTranscript(sender) {
    const time = formatTime(secondsElapsed);
...
function fetchStreamedAIReply(userText, onDelta) {
    return new Promise((resolve, reject) => {
...
function calculateConfidence(transcript) {
